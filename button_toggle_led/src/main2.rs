...
fn main() -> ! {
	...
	let mut led = pins.d3.into_output();
        let mut button = pins.d7.into_input();

	let mut lastButtonState = PinState::Low;
	let mut currentButtonState = PinState::Low;

	loop {
		lastButtonState = currentButtonState;
		currentButtonState = button.is_high().unwrap();

		if(lastButtonState == PinState::High && currentButtonState == PinState::Low) {
			led.set_high().unwrap();
		} else if(lastButtonState == PinState::Low && currentButtonState == PinState::High) {
			led.set_low().unwrap();
		}
	}
}
